...
import { detectConflictingPaths, printCustomRoutes, printTreeView, copyTracedFiles, isReservedPage, isAppBuiltinPage, collectRoutesUsingEdgeRuntime, collectMeta } from './utils';
...
export default async function build(dir, reactProductionProfiling = false, debugOutput = false, debugPrerender = false, noMangling = false, appDirOnly = false, bundler = Bundler.Turbopack, experimentalBuildMode, traceUploadUrl) {
    ...
    try {
        ...
        await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            if (hasMiddlewareFile) {
                if (hasProxyFile) {
                    throw Object.defineProperty(new Error(`Both "${MIDDLEWARE_FILENAME}" and "${PROXY_FILENAME}" files are detected. Please use "${PROXY_FILENAME}" instead.`), "__NEXT_ERROR_CODE", {
                        value: "E866",
                        enumerable: false,
                        configurable: true
                    });
                }
                Log.warn(`The "${MIDDLEWARE_FILENAME}" file convention is deprecated. Please use "${PROXY_FILENAME}" instead.`);
            }
       ...
       */ const sourcePages = new Map();
            const routesManifest = nextBuildSpan.traceChild('generate-routes-manifest').traceFn(()=>{
                ...
                return {
                    ...
                    rsc: {
                        ...
                        clientParamParsing: // NOTE: once this is the default for `clientSegmentCache`, this
                        // should exclusively be based on the `clientSegmentCache` flag.
                        config.experimental.clientParamParsing ?? false,
                        clientParamParsingOrigins: config.experimental.clientParamParsing ? config.experimental.clientParamParsingOrigins : undefined,
            ...
            await recordFrameworkVersion("15.6.0-canary.58");
            ...
            if (postCompileSpinner) {
                ...
                postCompileSpinner.setText(`Collecting page data in ${hrtimeDurationToString(collectingPageDataEnd)}`);
            ...
            const middlewareFile = rootPaths.find((p)=>p.includes(MIDDLEWARE_FILENAME) || p.includes(PROXY_FILENAME));
            ...
            if (process.env.__NEXT_TEST_MODE && process.env.NEXT_PRIVATE_DEBUG_CACHE_ENTRY_HANDLERS) {
                requiredServerFilesManifest.files.push(path.relative(dir, path.isAbsolute(process.env.NEXT_PRIVATE_DEBUG_CACHE_ENTRY_HANDLERS) ? process.env.NEXT_PRIVATE_DEBUG_CACHE_ENTRY_HANDLERS : path.join(dir, process.env.NEXT_PRIVATE_DEBUG_CACHE_ENTRY_HANDLERS)));
            }
            const features = [
                {
                        ...
                        exportPathMap: (defaultMap)=>{
                            ...
                            sortedStaticPaths.forEach(([originalAppPath, routes])=>{
                                ...
                                routes.forEach((route)=>{
                                    ...
                                    // completely static.
                                    !(config.experimental.clientSegmentCache && config.experimental.clientParamParsing)) {
                                        return;
                                    }
                                    defaultMap[route.pathname] = {
                    ...
                    sortedStaticPaths.forEach(([originalAppPath, prerenderedRoutes])=>{
                        ...
                        for (const route of staticPrerenderedRoutes){
                            ...
                            if (cacheControl.revalidate !== 0) {
                                ...
                                // should add this key.
                                if (!isAppRouteHandler && isAppPPREnabled && // Don't add a prefetch data route if we have both
                                ...
                                // this route.
                                !(config.experimental.clientSegmentCache && config.experimental.clientParamParsing && isRoutePPREnabled)) {
                        ...
                        if (!hasRevalidateZero && isDynamicRoute(page)) {
                            ...
                            for (const route of dynamicPrerenderedRoutes){
                                ...
                                if (!isAppRouteHandler && isAppPPREnabled) {
                                    if (// Don't add a prefetch data route if we have both
                                    // clientSegmentCache and clientParamParsing enabled. This is
                                    // because we don't actually use the prefetch data route in
                                    // this case. This only applies if we have PPR enabled for
                                    // this route.
