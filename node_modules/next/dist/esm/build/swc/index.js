    ...
    async function serializeNextConfig(nextConfig, projectPath) {
        ...
        if (nextConfigSerializable.images.loaderFile) {
            nextConfigSerializable.images = {
                ...nextConfigSerializable.images,
                loaderFile: './' + path.relative(projectPath, nextConfigSerializable.images.loaderFile)
            };
        }
        // cacheHandler can be an absolute path, we need it to be relative for turbopack.
        if (nextConfigSerializable.cacheHandler) {
            nextConfigSerializable.cacheHandler = './' + (path.isAbsolute(nextConfigSerializable.cacheHandler) ? path.relative(projectPath, nextConfigSerializable.cacheHandler) : nextConfigSerializable.cacheHandler);
        }
        if ((_nextConfigSerializable_experimental = nextConfigSerializable.experimental) == null ? void 0 : _nextConfigSerializable_experimental.cacheHandlers) {
            nextConfigSerializable.experimental = {
                ...nextConfigSerializable.experimental,
                cacheHandlers: Object.fromEntries(Object.entries(nextConfigSerializable.experimental.cacheHandlers).filter(([_, value])=>value != null).map(([key, value])=>[
                        key,
                        './' + (path.isAbsolute(value) ? path.relative(projectPath, value) : value)
                    ]))
            };
        }
        ...
        nativeBindings = {
            ...
            turbo: {
                ...
                startTurbopackTraceServer (traceFilePath, port) {
                    Log.warn(`Turbopack trace server started. View trace at https://trace.nextjs.org${port != null ? `?port=${port}` : ''}`);
