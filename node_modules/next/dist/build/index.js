...
async function build(dir, reactProductionProfiling = false, debugOutput = false, debugPrerender = false, noMangling = false, appDirOnly = false, bundler = _bundler.Bundler.Turbopack, experimentalBuildMode, traceUploadUrl) {
    ...
    try {
        ...
        await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            const hasProxyFile = rootPaths.some((p)=>p.includes(_constants.PROXY_FILENAME));
            if (hasMiddlewareFile) {
                if (hasProxyFile) {
                    throw Object.defineProperty(new Error(`Both "${_constants.MIDDLEWARE_FILENAME}" and "${_constants.PROXY_FILENAME}" files are detected. Please use "${_constants.PROXY_FILENAME}" instead.`), "__NEXT_ERROR_CODE", {
                        value: "E866",
                        enumerable: false,
                        configurable: true
                    });
                }
                _log.warn(`The "${_constants.MIDDLEWARE_FILENAME}" file convention is deprecated. Please use "${_constants.PROXY_FILENAME}" instead.`);
            }
       ...
       */ const sourcePages = new Map();
            ...
            await (0, _builddiagnostics.recordFrameworkVersion)("15.6.0-canary.58");
            ...
            if (postCompileSpinner) {
                ...
                postCompileSpinner.setText(`Collecting page data in ${(0, _durationtostring.hrtimeDurationToString)(collectingPageDataEnd)}`);
            ...
            const middlewareFile = rootPaths.find((p)=>p.includes(_constants.MIDDLEWARE_FILENAME) || p.includes(_constants.PROXY_FILENAME));
